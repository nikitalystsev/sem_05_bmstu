    ...
    def gen_data(self):
        ...
        with open("data.csv", "w", newline='') as file:
            ...
            for i in range(self.count_record):
                fio = faker.name()  # фио
                weight = rd.randint(45, 100) * 1000  # вес
                height = rd.randint(155, 195)
                gender = "Ж" if fio.split()[0][-1] in "аеиоуыэюя" else "М"
                vital_capacity = round(rd.uniform(2.5, 3), 2) * 1000 if gender == "Ж" else (
                            round(rd.uniform(3.5, 4), 2) * 1000)
                hand_strength = rd.randint(25, 33) if gender == "Ж" else rd.randint(35, 50)
                heart_rate = rd.randint(60, 90)
                systolic_pressure = rd.randint(100, 140)
                diastal_pressure = rd.randint(50, 100)
                martin_sample = rd.randint(3, 12) * 10

                all_data_doc.append([fio, weight, height, vital_capacity, hand_strength, heart_rate,
                                     systolic_pressure, diastal_pressure, martin_sample])
